...
public class LiteModeSettingsActivity extends BaseFragment {
    ...
    private void updateItems() {
        ...
        if (expanded[2]) {
            ...
            if (Build.VERSION.SDK_INT >= 31 && (SharedConfig.getDevicePerformanceClass() >= SharedConfig.PERFORMANCE_CLASS_AVERAGE || BuildVars.DEBUG_PRIVATE_VERSION)) {
...
//                updateSlider(slideChooseView);
    ...
    private class SwitchCell extends FrameLayout {
        ...
        private int preprocessFlagsCount(int flags) {
            ...
            if (!(Build.VERSION.SDK_INT >= 31 && (SharedConfig.getDevicePerformanceClass() >= SharedConfig.PERFORMANCE_CLASS_AVERAGE || BuildVars.DEBUG_PRIVATE_VERSION)) && (flags & LiteMode.FLAG_LIQUID_GLASS) > 0) {
                count--;
            }
